...
class RateLimiter extends RateLimitEventEmitter {
  ...
  async handle (request, h) {
    ...
    if (await this.isBlocked(request)) {
      return await this.doHandleExceeded()
    }
  ...
  async isBlocked(request) {
    ...
    const key =  await this.requestIdentifier(request)
    return !!(await this.redis.get(`blocked_${key}`))
  }
  ...
  async handleExceeded (request, h) {
    if (this.blockDuration) {
      const key =  await this.requestIdentifier(request)
      await this.redis.set(`blocked_${key}`, `blocked_${key}`);
      await this.redis.expire(`blocked_${key}`, this.blockDuration);
    }
    await this.fireExceededEvent(request)

    return this.doHandleExceeded(request, h)
  }
  async doHandleExceeded (request, h) {
